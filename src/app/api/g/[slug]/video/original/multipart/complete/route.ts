...
export async function POST(req: NextRequest, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const [gallery] = await db
    .select({ id: galleries.id, oldKey: galleries.heroVideoOriginalKey })
    .from(galleries)
  ...
  await db
    .update(galleries)
    .set({
      heroVideoOriginalKey: body.key,
      heroVideoOriginalMime: body.mime,
      heroVideoOriginalSizeBytes: body.sizeBytes ?? null,
    })
